...
# hk_log_repeat_50_true.txt  [02:55<00:00,  6.03s/it]
...
def calculate_similarity_matrix(active_nodes, node_states):
    ...
    state_diff_matrix_df = pd.DataFrame(state_diff_matrix, index=active_nodes,
                                        columns=active_nodes)

    # 合并嵌入相似度和状态差异
    similarity_matrix = state_diff_matrix_df

    return similarity_matrix


def find_neighbors_bool_matrix(active_nodes, opinions, similarity_threshold):
    # 计算相似性矩阵
    similarity_matrix = calculate_similarity_matrix(active_nodes, opinions)
...
# 读取保存最优参数的 CSV 文件
best_params_df = pd.read_csv('D:/研究生/论文/毕业论文/代码/网络构建/直播平台网络模型/jupyter/best_parameters_and_thresholds_l1.csv')
num_iterations = 180  # 迭代次数

log_file = "./对比实验/log_hk_l1.txt"  # 设置日志文件名
...
for start_idx in tqdm(range(0, 31, 1)):
    ...
    for seed in tqdm(range(0, 100)):
        ...
        for _ in range(num_iterations):
            ...
            bool_matrix = find_neighbors_bool_matrix(active_nodes, opinions, similarity_threshold)
            for i in active_nodes:
                ...
                if len(neighbor_indices) > 0:
                    ...
                    opinions[i] = neighbor_opinions_mean
